                ...
                plantTasksList.map(({ user_plant, tasks }) => (
                    <ThemedView key={user_plant.id} style={styles.card}>
                        <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 16, justifyContent: 'space-between' }}>
                            {user_plant.plant.image && (
                                <Image
                                    source={{ uri: user_plant.plant.image }}
                                    style={{ width: 50, height: 50, borderRadius: 100 }}
                                />
                            )}
                            <ThemedText type="title2">{user_plant.plant.common_name
                                ? user_plant.plant.common_name.charAt(0).toUpperCase() + user_plant.plant.common_name.slice(1)
                                : ""}</ThemedText>
